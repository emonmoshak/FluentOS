...
interface BlobVisualizerProps {
  ...
  state: 'listening' | 'speaking' | 'connecting' | 'idle' | 'muted';
}
...
export const BlobVisualizer: React.FC<BlobVisualizerProps> = ({ volume, state }) => {
  ...
  const getBlobColor = () => {
    switch (state) {
        case 'muted': return 'bg-gray-300';
        case 'speaking': return 'bg-yellow-400';
  ...
  return (
    <div className="relative flex items-center justify-center w-80 h-80">
      {/* Outer Glow / Ripple - hidden if muted */}
      {state !== 'muted' && (
        <div 
      ...
      {/* Main Blob */}
      <div 
        className={`relative w-48 h-48 rounded-full ${getBlobColor()} blob-shadow flex items-center justify-center transition-all duration-300 ease-linear z-10`}
        style={{
      ...
      {/* Idle Breathing Animation Ring (when listening and not muted) */}
      {state === 'listening' && (
